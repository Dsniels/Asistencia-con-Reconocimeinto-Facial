...
export default function TabTwoScreen() {
  ...
  const fetch = async () => {
    try {
      ...
      if (v) {
        ...
        setAsistencia(data);
        const total = await store.getRegisteredStudentsByGroup(groups[0]);
        setTotalAlumnos(total.count)
      } else {
        setAsistencia({});
      }
    } catch (error) {
  ...
  const handlerChangeGroup = async (itemValue: string) => {
    setSelectedGroup(itemValue);
    const total = await store.getRegisteredStudentsByGroup(itemValue.toString());
    setTodayTotal(Object.entries(filteredAsistencia).flat()[1].length)
    setTotalAlumnos(total.count)
  };
      ...
      >
        <View className="flex-1 m-3 shadow-md shadow-cyan-900  p-5 rounded-3xl">
          <View className="flex-row justify-between items-center">
            ...
            >
              <Text className="text-sm text-gray-500">Eliminar Datos</Text>
            </Pressable>
          </View>
          <View className="items-center content-between justify-center">
            <View className="flex-row items-center  mb-2">
              ...
              <Picker
                ...
                onValueChange={(itemValue) => handlerChangeGroup(itemValue)}
                style={styles.picker}
              >
                {groups.map(
                  (group) =>
        ...
        <View className="flex-1 p-5 m-3 mt-4 mb-9  shadow-xl shadow-cyan-900 min-h-32 rounded-3xl">
          <View className="flex-row justify-between items-center">
            ...
            >
              <Text className="text-sm text-gray-500">Exportar</Text>
            </Pressable>
          </View>
          {asistencia && Object.keys(filteredAsistencia).length > 0 ? (
            Object.entries(filteredAsistencia).map(([date, names]) => (
              <View key={date} style={styles.dateContainer}>
                {names.map((name, index) => (
                  <Pressable key={index} onPress={() => handlePress(name)}>
                    <Text className="text-lg m-2 ml-4">{name}</Text>
                  </Pressable>
                ))}
              </View>
            ))
          ...
          <Text className="text-sm text-gray-500"> Total de asistencias: {todayTotal}/{totalAlumnos} </Text>
        </View>
      </ScrollView>
      <Modal
        transparent
